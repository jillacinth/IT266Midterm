...
void Cmd_HelpScreen_f(const idCmdArgs& args) {
	gameLocal.Printf("Move with WASD and jump with SPACE\n Collect all the stars to finish the level \n Use and collect weapons to fend off enemies and spawn powerups \n avoid the enemies to survive \n Good luck!");
}
...
void idGameLocal::InitConsoleCommands(void) {
	...
	*/
	...
	// RITUAL END
		...
		//help Screen
	cmdSystem->AddCommand("helpScreen", Cmd_HelpScreen_f, CMD_FL_GAME, "Help Screen");

	//start game
	cmdSystem->AddCommand("startGame", Cmd_StartGame_f, CMD_FL_GAME, "Starts game in map");
}
